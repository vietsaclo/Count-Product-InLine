...
def fun_renames_by_count(_dir: str, count: int= 0, prefix: str= ''):
    fileNames = fun_getFileNames(_dir)
    _count = count
    for f in fileNames:
        os.rename(_dir + '/' + f, _dir + '/' + prefix + str(_count) + f[len(f) - 4:])
        _count += 1

def fun_reduceSizeImage(image, reduceSizeFloat: float= 0.5):
    h, w, _ = image.shape
    image = cv2.resize(image, (int(w * reduceSizeFloat), int(h * reduceSizeFloat)))
    return image

def fun_reduceQuality(frame, size: int = 10):
    frame = Image.fromarray(frame)
    frame = frame.reduce(10)
    return np.asarray(frame)

def fun_skipFrame(cap, count: int= 5):
    while count > -1:
        cap.read()
        count -= 1
...
'''
def fun_putText_prediction(img, class_id, confidence, x, y, x_plus_w, y_plus_h):
